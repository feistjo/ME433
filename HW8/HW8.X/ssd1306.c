...
//draw a char. Call update() to push to the display)
void ssd1306_drawChar(unsigned char x, unsigned char y, unsigned char c) {
    //char* c_arr = ASCII[c-20];//{0x01, 0x00, 0x10, 0x00, 0x00};//ASCII[c-20];
    for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 8; j++) {
            ssd1306_drawPixel(x + i, y + j, (ASCII[c-0x20][i] >> j) & 0b00000001);
        }
    }
}

//draw a string. Call update() to push to the display)
void ssd1306_drawStr(unsigned char x, unsigned char y, unsigned char* s) {
    int counter = 0;
    while (s[counter] != '\0') {
        ssd1306_drawChar(x + 5*counter, y, s[counter]);
        counter++;
    }
}
