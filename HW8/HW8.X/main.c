...
int main() {
    ...
    while (1) {
        if(_CP0_GET_COUNT() >= 24000000) {
            ...
            myVar++;
            //ssd1306_drawStr(0,0,"Hello World!");
            _CP0_SET_COUNT(0);
        }
        ...
        char message[50];
        sprintf(message, "my var = %d", myVar);
        ssd1306_drawStr(10, 10, message);
        
        long curr_cp0 = _CP0_GET_COUNT();
        
        float fps = curr_cp0>prev_cp0?curr_cp0-prev_cp0:(curr_cp0+24000000)-prev_cp0;
        fps = (float)24000000/(float)fps;
        //_CP0_SET_COUNT(0);
        prev_cp0 = curr_cp0;
        sprintf(message, "FPS: %f", fps);
        ssd1306_drawStr(10, 20, message);
