...
float comp_filt(uint8_t * data, float prev_pitch, float DT) {
    float A_x = conv_xXL(data);
    float A_y = conv_yXL(data);
    float A_z = conv_zXL(data);
    
    float w_x = conv_xG(data); //w=omega
    float w_y = conv_yG(data);
    float w_z = conv_zG(data);
    
    float theta_XL = atan2f(A_x, A_z);
    
    float new_pitch = (w_x*DT) + prev_pitch;
    return COMP_FILT_COEFF * theta_XL + (1-COMP_FILT_COEFF) * new_pitch;
}